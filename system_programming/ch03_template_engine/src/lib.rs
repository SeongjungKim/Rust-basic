...
pub fn get_contents_type(input_line: &str) -> ContentType {
    let is_tag_expression = check_matching_pair(&input_line, "{%", "%}");
    let is_for_tag = (check_symbol_string(&input_line, "for") && check_symbol_string(&input_line, "in"))
...
mod tests {
    ...
    fn check_literal_test() {
        ...
        assert_eq!(
            ContentType::Literal(s.to_string()), 
            get_contents_type(s)
        );
    }
    ...
    fn check_template_var_test() {
        ...
        assert_eq!(
            ...
            get_contents_type("Hi {{name}} ,welcome")
        );
    }
    ...
    fn check_for_tag_test() {
        assert_eq!(
            ...
            get_contents_type("{% for name in names %}, welcome")
        );
    }
    ...
    fn check_get_index_for_symbol_test() {
        assert_eq!(
            (true, 3),
            get_index_for_symbol("Hi {name}, wellcome", '{')
        );
    }
}