...
/// a list.
///
pub struct DynamicArray<T> where T: Sized + Clone {
    buf: Box<[Node<T>]>,
    cap: usize,
    pub length: usize,
}
impl<T> DynamicArray<T> where T: Sized + Clone {
    ///
...
impl<T> Index<usize> for DynamicArray<T> where T: Sized + Clone {
    type Output = Node<T>;
...
impl<T> Clone for DynamicArray<T> where T: Sized + Clone {
    fn clone(&self) -> Self {
        DynamicArray {
            buf: self.buf.clone(),
            cap: self.cap,
