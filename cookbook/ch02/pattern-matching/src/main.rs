...
fn destructuring_match(app: MyApp) -> String {
    match app {
        ...
        MyApp { user_type: _, secret_user_id: uid, .. } =>
            format!("A regular user with id >{}<, individual backgrounds not supported", uid),
    }
}

fn literal_match(choice: usize) -> String {
    match choice {
        0 | 1 => "zero or on".to_owned(),
...
fn main() {
    ...
    println!("Matching on tuple: {}", tuple_match((0, 10, 0, 100)));
    println!();
    ...
    println!("Guarded matching (latd): {}", guarded_match(not_so_early));
    println!();
